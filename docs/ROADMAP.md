...
- Extension navigateur : préparation publication **1.6.1** en cours. Les ZIP Firefox/AMO et Chrome Web Store sont déjà générés depuis la même source canonique ; nom public encore à choisir avant soumission. Le userscript possède déjà un canal d'auto-update via `@updateURL` / `@downloadURL` sur `main`, à condition d'incrémenter `@version` à chaque publication.
